...
#' Write/update polls and trends and generate plot
#'
#' @param make_plot Logical, indicating whether generate plot. Default is TRUE.
#'
#' @examples
dataland2024_polls_trends <- function(make_plot =T){
  ...
  error = function(e) {
    cat(Sys.time(), paste0("An error occured while updating/writing polls:", e, 
                           "\n"), 
        file = log_file, append = TRUE)
    message(paste0("Error: Something went wrong while updating/writing polls, check ", 
                   log_file, "."))
  })
  ...
  tryCatch({  
    ...
    if(make_plot == T){
      ...
      ggsave(filename = "plots/polls_trends_plot.png", 
             ...
             width = 12, height = 6, 
             bg='#ffffff')
      print("Polls & trends plot is generated and saved in the 'plots' folder.")
    }
  }, 
  
  error = function(e) {
    cat(Sys.time(), paste0("An error occured while generating polls/trends plot:", e, 
                           "\n"), 
        file = log_file, append = TRUE)
    message(paste0("Error: Something went wrong while generating polls/trends plot, check ", 
                   log_file, "."))
  })  
